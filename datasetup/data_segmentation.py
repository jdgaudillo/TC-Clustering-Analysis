...
def hierarchicalClustering(data):
    ...
    ms.fit(copy)
    lbs = ms.labels_
    data.loc[:, 'Hierachical Clustering Label'] = lbs
...
def silhouetteAnalysis(data, name, add):
    ...
    for n_cluster in k:
        ...
        sil_score.append((n_cluster, silhouette_avg))
    ...
    np.savetxt('silhouette_scores.txt', sil_score)
